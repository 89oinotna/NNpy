...
def read_monk(filename):
    ...
    vl_labels = test_data.pop('class').to_frame().values
    train_data = OneHotEncoder().fit_transform(train_data).toarray().astype(np.float32)
    vl_data = OneHotEncoder().fit_transform(test_data).toarray().astype(np.float32)
    ...
    return train_data, train_labels, vl_data, vl_labels, test_data, test_labels
